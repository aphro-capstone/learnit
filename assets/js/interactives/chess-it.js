...
$(function() {		 
    ...
    // Draw the board game
        let size = 8; 
        let rw = $('<div class="row"></div>');
        let box_el = $('<div class="box" piece=""></div>');

        //Draw board
        for(var y = 0; y < size; y++) { 
            let rww = rw.clone();
            for(var x = 0; x <  size;x++ ){
                let boxxx = box_el.clone();
                    boxxx.attr('id', 'box-' + x + '-' + y)
                rww.append( boxxx );
            }
        ...
        for(var i = 0; i < 8; i++) {
            ...
            for(var j = 0; j < 8; j++) {
                    var box_ = $('#box-' + i + '-' + j);
                    if((i + j) % 2 !== 0) {
                        box_.addClass('light-box');
                    } else {
                        box_.addClass('dark-box');
                    }
                    setNewBoard(box_, i, j); //Set up all chess pieces
            }
        }
        setColor();
        setTheme();
